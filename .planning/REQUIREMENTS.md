...
- [x] **FOUND-02**: CLI tools handle template filling from `.planning/` templates
- [x] **FOUND-03**: CLI tools provide atomic git commit operations (temp file + rename pattern for state safety)
- [x] **FOUND-04**: CLI tools provide dependency analysis for plan execution ordering
...
- [x] **FOUND-07**: ~12 skill entry points installed under `.claude/skills/dan-*/` with frontmatter invocation control
...
- [x] **FOUND-10**: Progress tracking in STATE.md with atomic updates per task completion
...
- [x] **LOOP-01**: `/dan:plan` creates executable plan with objective, acceptance criteria, tasks, and boundaries
- [x] **LOOP-02**: Plans are sized for single context window (2-3 tasks per plan)
- [x] **LOOP-03**: `/dan:apply` executes plan tasks with fresh-context executor agent
- [x] **LOOP-04**: Execute/Qualify separation — independent qualifier agent re-reads output and grades against acceptance criteria
...
- [x] **LOOP-07**: `/dan:unify` creates SUMMARY.md documenting what was built, comparing plan vs actual
- [x] **LOOP-08**: Mandatory loop closure — every PLAN gets a SUMMARY, no orphan plans ever
- [x] **LOOP-09**: Plan lifecycle states: DRAFT → APPROVED → IN_PROGRESS → COMPLETED/ABANDONED
- [x] **LOOP-10**: Atomic task checkpointing — task status saved to state after each completion
...
- [x] **QUAL-02**: Verification is automated — no human intervention required
- [x] **QUAL-03**: `/dan:bugsweep` runs recursive audit loop: find issues → fix → re-verify → loop until clean
